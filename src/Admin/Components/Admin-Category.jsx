...
var ProductCategory =()=>
    {
                ...
                try
                {
                    ...
                    if(resp.ok)
                    {
                        ...
                        else if(result.statuscode===1)
                        {
                            ...
                            const ids = result.membsdata.map(item => item.Uploader);
                            setIds(ids);
                        }
                    }
                    else
        ...
        var fetchRegisters =async()=>
            {
                try
                {
                    var data = {IDs:RegIds};
                    const resp = await fetch(`${process.env.REACT_APP_APIURL}/register-names-byids`,
                    {
                        method:"post",
                        body:JSON.stringify(data),
                        headers:{'Content-type':'application/json'}
                        ...
                        <div className="m-p-s-head">
                            <div className="flex justify-content-between align-items-center">
                                ...
                                <div className="for-new-pro">
                                    <Link className="newitem-btn" to={`/admin/add-category`}><i class="fa-solid fa-plus"></i> Add Category</Link>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <tbody>
                                            {
                                                ShowProductData?.map((data, i) =>
                                                    ...
                                                    <tr key={i}>
                                                        <td><img src={`/CategoryUploads/${data.Image}`} /></td>
                                                        <td className="title-table-bold position-relative">
                                                        ...
                                                        <td>{Registers?.filter((e)=> e._id === data.Uploader)[0]?.name}</td>
                                                        <td title={
